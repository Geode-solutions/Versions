...
export default async function cpp_deploy(octokit, ref) {
  ...
  const og_stochastic = og.then(() => {
    return deploy_repository(octokit, "Geode-solutions", "OpenGeode-Stochastic", ref);
  });
  const og_io = og.then(() => {
    return deploy_repository(octokit, "Geode-solutions", "OpenGeode-IO", ref);
  });
  const og_geosciences = og.then(() => {
    return deploy_repository(octokit, "Geode-solutions", "OpenGeode-Geosciences", ref);
  });
  const og_geosciencesio = Promise.all([og, og_io, og_geosciences]).then(() => {
    return deploy_repository(octokit, "Geode-solutions", "OpenGeode-GeosciencesIO", ref);
  });
  ...
  const g_viewables = Promise.all([og_geosciencesio, g_common]).then(() => {
    return deploy_repository(octokit, "Geode-solutions", "Geode-Viewables_private", ref);
  });
  const g_conversion = g_common.then(() => {
    return deploy_repository(octokit, "Geode-solutions", "Geode-Conversion_private", ref);
  });
  const g_background = g_common.then(() => {
    return deploy_repository(octokit, "Geode-solutions", "Geode-Background_private", ref);
  });
  const g_simplex = Promise.all([og_io, og_inspector, g_background]).then(() => {
    return deploy_repository(octokit, "Geode-solutions", "Geode-Simplex_private", ref);
  });
  const g_hybrid = g_simplex.then(() => {
    return deploy_repository(octokit, "Geode-solutions", "Geode-Hybrid_private", ref);
  });
  const g_hybrid_geosciences = Promise.all([
    ...
    return deploy_repository(octokit, "Geode-solutions", "Geode-Hybrid_Geosciences_private", ref);
  });
  const g_explicit = Promise.all([og_inspector, g_conversion, g_background]).then(() => {
    return deploy_repository(octokit, "Geode-solutions", "Geode-Explicit_private", ref);
  });
  const g_explicit_geosciences = Promise.all([og_geosciences, g_explicit]).then(() => {
    return deploy_repository(octokit, "Geode-solutions", "Geode-Explicit_Geosciences_private", ref);
  });
  const g_implicit = Promise.all([g_explicit, g_simplex]).then(() => {
    return deploy_repository(octokit, "Geode-solutions", "Geode-Implicit_private", ref);
  });
