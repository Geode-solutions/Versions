type HookMethod<Options, Result> = (options: Options) => Result | Promise<Result>;

type BeforeHook<Options> = (options: Options) => void | Promise<void>;
type ErrorHook<Options, Error> = (error: Error, options: Options) => unknown | Promise<unknown>;
type AfterHook<Options, Result> = (result: Result, options: Options) => void | Promise<void>;
...
type GetType<TStore extends TypeStore, TKey extends TypeStoreKey> = TKey extends keyof TStore
  ? TStore[TKey]
  : any;
...
> {
  ...
  <Name extends HookName>(
    name: Name | Name[],
    hookMethod: HookMethod<GetType<HooksType[Name], "Options">, GetType<HooksType[Name], "Result">>,
  ...
  error<Name extends HookName>(
    name: Name,
    errorHook: ErrorHook<GetType<HooksType[Name], "Options">, GetType<HooksType[Name], "Error">>,
  ): void;
  /**
  ...
  after<Name extends HookName>(
    name: Name,
    afterHook: AfterHook<GetType<HooksType[Name], "Options">, GetType<HooksType[Name], "Result">>,
  ): void;
  /**
  ...
  wrap<Name extends HookName>(
    name: Name,
    wrapHook: WrapHook<GetType<HooksType[Name], "Options">, GetType<HooksType[Name], "Result">>,
  ): void;
  /**
   ...
   * Public API
   */
  api: Pick<HookCollection<HooksType>, "before" | "error" | "after" | "wrap" | "remove">;
}
...
export interface HookSingular<Options, Result, Error> {
   ...
   * Public API
   */
  api: Pick<HookSingular<Options, Result, Error>, "before" | "error" | "after" | "wrap" | "remove">;
}

type Collection = new <
  HooksType extends Record<string, TypeStore> = Record<string, TypeStore>,
>() => HookCollection<HooksType>;
type Singular = new <Options = unknown, Result = unknown, Error = unknown>() => HookSingular<
  Options,
  Result,
  Error
>;
