...
export default async function web_deploy(octokit, ref) {
  ...
  const ogw_back = ogw_microservice.then(() => {
    return deploy_repository(octokit, "Geode-solutions", "OpenGeodeWeb-Back", ref);
  });
  ...
  const vease_modeling_back = ogw_back.then(() => {
    return deploy_repository(octokit, "Geode-solutions", "Vease-Modeling-Back", ref);
  });
  ...
  const ogw_viewer = ogw_microservice.then(() => {
    return deploy_repository(octokit, "Geode-solutions", "OpenGeodeWeb-Viewer", ref);
  });
